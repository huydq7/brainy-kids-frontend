...
export default function ExamPage() {
  ...
  const handleSubmit = () => {
    ...
    router.push(`/test/${params.id}/result?answers=${encodedAnswers}`);
  };

  const handleNextPart = () => {
    if (!exam) return;
    const currentPartNum = parseInt(currentPart);
    const nextPart = exam.parts.find(
  ...
  const currentPartNum = parseInt(currentPart);
  const isLastPart =
    currentPartNum === Math.max(...exam.parts.map((p) => p.partNumber));
  const allQuestionsAnswered = answeredQuestions === totalQuestions;
  const canSubmit = isLastPart || allQuestionsAnswered;

  const isFirstPart =
    currentPartNum === Math.min(...exam.parts.map((p) => p.partNumber));
                  ...
                  <div className="flex justify-between items-center mt-8 pt-6 border-t">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-600">
                        Part {part.partNumber} of {exam.parts.length}
                      </span>
                    ...
                    {isLastPartAvailable ? (
                      ...
                      <div className="w-[120px]" /> // Placeholder for alignment
                    )}
                  </div>
                </Card>
              </TabsContent>
